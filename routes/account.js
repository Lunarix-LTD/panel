...
router.get('/account', async (req, res) => {
  ...
  res.render('account', { req, user: req.user, users, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
});
...
router.get('/enable-2fa', async (req, res) => {
    try {
        ...
        const secret = speakeasy.generateSecret({
            length: 20,
            name: `Lunarix (${currentUser.username})`,
            issuer: 'Lunarix'
        });
        ...
        qrcode.toDataURL(secret.otpauth_url, async (err, data_url) => {
            ...
            res.render('enable-2fa', { qrCode: data_url, req, user: req.user, users, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
        });
    } catch (error) {
