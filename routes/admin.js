...
router.get('/admin/apikeys', isAdmin, async (req, res) => {
  try {
    ...
    res.render('admin/apikeys', { req, user: req.user, apiKeys, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
  } catch (error) {
...
router.get('/admin/nodes', isAdmin, async (req, res) => {
  ...
  res.render('admin/nodes', { req, user: req.user, nodes, set, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
});
...
router.get('/admin/settings', isAdmin, async (req, res) => {
  res.render('admin/settings/appearance', { req, user: req.user, settings: await db.get('settings'), name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
});
...
router.get('/admin/instances', isAdmin, async (req, res) => {
  ...
  res.render('admin/instances', { req, user: req.user, instances, images, nodes, users, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
});
...
router.get('/admin/instances/:id/edit', isAdmin, async (req, res) => {
  ...
  res.render('admin/instance_edit', { req, user: req.user, instance, images, users, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
})

router.get('/admin/users', isAdmin, async (req, res) => {
  let users = await db.get('users') || [];

  res.render('admin/users', { req, user: req.user, users, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
});
...
router.get("/admin/node/:id", async (req, res) => {
    ...
    res.render('admin/node', { req, node, user: req.user, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
});
...
router.get('/admin/images', isAdmin, async (req, res) => {
  ...
  res.render('admin/images', { req, user: req.user, images, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
});
...
async function deleteInstance(instance) {
  try {
    await axios.get(`http://Lunarix:${instance.Node.apiKey}@${instance.Node.address}:${instance.Node.port}/instances/${instance.ContainerId}/delete`);
...
router.get('/admin/auditlogs', isAdmin, async (req, res) => {
  try {
    ...
    res.render('admin/auditlogs', { req, user: req.user, audits, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false });
  } catch (err) {
