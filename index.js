...
app.listen(config.port, () => log.info(`lunarix is listening on port ${config.port}`));

app.get('*', async function(req, res){
  res.render('errors/404', { req, name: await db.get('name') || 'Lunarix', logo: await db.get('logo') || false })
});